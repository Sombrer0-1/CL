...
- `light24/coverage.csv`：第一阶段冻结源码与配置身份逐格覆盖，未跑/失败不填零。
- `light24/attempts.csv`：第一阶段所有可读取的运行尝试，保留失败和中断。
...
- `light24/readiness.json`：第一阶段当时的代码与执行入口验证摘要，不是科学结论。
...
旧 `tabulate_context.py`、`pair_speedup.py`、`extract_*` 等仅汇总非light24运行；light24 使用其阶段专用入口。原始记录仍在 `runs/<run_id>/`，保留失败、低准确率、GSS崩塌和预取负收益。本机 `effectiveness_v3_*` 跑数已删除。
...
- `pressure_v2/progress.json`、`pressure_v2/attempts.csv`：开发+正式记录摘要；原始 runs 仍可能在 `runs/`（WSL 拷贝）。
- `pressure_v2/readiness.json`：实现准备摘要，不是正式科学结论。

## 新阶段材料

`effectiveness_v3/` 现有实现审计、设计自审和测试脚本，**不是**方法有效性报告，也没有可迁移的 r1 校准。换机后在新宿主生成 `<revision>/` 正式报告。
