...
**换机说明（A26）：** 本文记录源码修补。其中 pytest 计数、envcheck 和 GPU OOM 诊断是当时 Linux + RTX 5090 上的检查，不能作为新宿主冻结或正式结果。换机后重跑测试与 envcheck。G0/G1 入口后来写入 `src/orion_repro/stages/`；G2–G4 须在新宿主重新校准。
...
| B03 | 环境检查只判断梯度存在；opt.step未更新参数也可能通过，Avalanche导入失败仍打印PASS | 检查loss/梯度/参数有限且实际参数变化；必需Avalanche/策略导入失败直接非零退出 | noop优化器拒绝、真实更新通过、模拟缺库拒绝；本机GPU envcheck通过 |
...
- 全套：**134 passed，10 warnings，5.96s**。原始输出：[validation_tests.txt](validation_tests.txt)。新增13项回归；库告警来自未配置logger及Avalanche replay update弃用。
- 本机 `orion_repro.envcheck`：Python3.11.16、torch2.11.0+cu128、torchvision0.26.0+cu128、Avalanche0.6.0，cuda:0有限梯度/参数实际更新通过。
- 真实CUDA失败阶段检查：进程allocator配额64MiB，前阶段峰值2MiB，评价阶段保留6MiB后请求128MiB触发OOM，失败记录峰值为6MiB而非上一阶段2MiB。见 [validation_gpu.json](validation_gpu.json)；复核脚本 [verify_failure_peak.py](verify_failure_peak.py)。没有改驱动或全局限制。
